...
// .vert | vs  for a vertex shader
// .tesc | tc  for a tessellation control shader
// .tese | te  for a tessellation evaluation shader
// .geom | gs  for a geometry shader
// .frag | fs  for a fragment shader
// .comp | cs  for a compute shader

const glslRegex = /^(.*(?:\.|_))(vs|tc|te|gs|fs|cs)(\.glsl)$/
const shRegex = /^(.*\.)(vs|tc|te|gs|fs|cs)$/
...
var extractShaderFilenameTokens = (shaderFilename) => {
  ...
  if (extGlslMatch) {
    ...
    linkTargets = ["vs", "tc", "te" ,"gs", "fs", "cs"].filter(x => x != baseShaderType)
    linkTargetPattern = `${baseFilename}{{type}}${baseExtension}`;
  } else if (extShMatch) {
    baseFilename = extShMatch[1];
    baseShaderType = extShMatch[2].slice(0, 2);
    ...
    // baseShaderType = extDefaultMatch[2];
    baseShaderType = ["vs", "tc", "te" ,"gs", "fs", "cs"]
    [["vert", "tesc", "tese", "geom", "frag", "comp"].indexOf(extDefaultMatch[2])]

    baseExtension = extDefaultMatch[2];

    linkTargets = ["vert", "tesc", "tese", "geom", "frag", "comp"].filter(x => x != baseExtension)
